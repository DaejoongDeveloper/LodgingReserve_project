				...
				<div class="content_main">
					...
					<div class="reserveLodging_div">
						...
						<div class="lodging_kind_div">
							예약상품 <span class="lodging_kind_div_kind"></span>종 <span
								class="lodging_kind_div_count"></span>개
						</div>
						<!-- 상품 테이블 -->
									...
									<tr>
										<td>
											<div class="image_wrap"
												data-lodgingid="${ol.imageList[0].lodgingId}"
										...
										<td class="lodging_table_price_td"><br> <fmt:formatNumber
												value="${ol.lodgingPrice}" pattern="#,### 원" />| 수량
											${ol.lodgingCount}개 <br> <fmt:formatNumber
												value="${ol.totalPrice}" pattern="#,### 원" /> <input
											type="hidden" class="individual_lodgingPrice_input"
											value="${ol.lodgingPrice}"> <input type="hidden"
											class="individual_lodgingCount_input"
											value="${ol.lodgingCount}"> <input type="hidden"
											class="individual_totalPrice_input"
											value="${ol.lodgingPrice * ol.lodgingCount}"> <input type="hidden" class="individual_lodgingId_input"
											value="${ol.lodgingId}"></td>
									</tr>
								</c:forEach>
		...
		function setTotalInfo() {
			...
			$(".lodging_table_price_td").each(function(index, element) {
						...
						totalCount += parseInt($(element).find(
								".individual_lodgingCount_input").val());
						// 총 종류
						totalKind += 1;
					});
...
$(".reserve_btn").on("click", function(){
	...
	/* 상품정보 */
		...
		$(".lodging_table_price_td").each(function(index, element){
			...
			let lodgingCount = $(element).find(".individual_lodgingCount_input").val();				let lodgingId_input = "<input name='reserves[" + index + "].lodgingId' type='hidden' value='" + lodgingId + "'>";
